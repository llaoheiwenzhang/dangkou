...
import { CalendarDays, Share2, Star, Edit3, ArrowUpRight } from "lucide-react";
...
export default function Buy() {
  ...
  useEffect(() => {
    fetch("/api/buy")
      ...
      .catch((err) => console.error("Failed to fetch buy items", err));
  }, []);
  ...
  return (
    <div className="pb-16 min-h-screen bg-[#f5f6f8]">
      <div className="bg-gradient-to-r from-[#70c946] to-[#4caf50] px-4 pt-8 pb-4 rounded-b-[20px] text-white flex justify-between items-end shadow-sm">
      ...
      <div className="sticky top-0 z-10 bg-[#f5f6f8] px-3 py-3 shadow-sm md:shadow-none">
        <div className="relative flex gap-2">
          ...
          />
          <button className="bg-[#4caf50] text-white px-5 py-2.5 rounded-lg font-medium shadow-sm text-sm">
            搜索
          </button>
        </div>
      </div>
      ...
      <div className="px-3 space-y-2">
        {items.map((item) => (
          <Link key={item._id} to={`/buy/${item._id}`} className="block bg-[#eafceb] p-3 rounded-lg border border-[#d2f0d4] shadow-[0_2px_8px_rgba(76,175,80,0.1)]">
            <div className="flex justify-between items-center mb-2">
              <span className="font-bold text-[16px] text-gray-800">档口: {item.shopName}</span>
              <span className="text-[#ff3b30] font-bold text-[20px]">¥{item.requestAmount}</span>
            </div>
